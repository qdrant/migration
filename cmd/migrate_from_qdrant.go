...
func (r *MigrateFromQdrantCmd) prepareTargetCollection(ctx context.Context, sourceClient *qdrant.Client, sourceCollection string, targetClient *qdrant.Client, targetCollection string) error {
	...
	if r.Migration.CreateCollection {
		...
		if exists {
			...
			shardingMethod := params.GetShardingMethod()
			if r.routesToTargetShard() {
				shardingMethod = qdrant.ShardingMethod_Custom
			}
			if err := targetClient.CreateCollection(ctx, &qdrant.CreateCollection{
				...
				ShardingMethod:         shardingMethod.Enum(),
			...
			if !r.routesToTargetShard() && params.GetShardingMethod() == qdrant.ShardingMethod_Custom {
...
func (r *MigrateFromQdrantCmd) processBatch(ctx context.Context, points []*qdrant.RetrievedPoint, targetClient *qdrant.Client, targetCollection string, wait bool) error {
	...
	for key, pts := range byShardKey {
		...
		if key != "" {
			if r.routesToTargetShard() {
				if _, ok := r.shardKeys.Load(key); !ok {
